...
function asignarFechaHoyInputDate() {
 ...
 async function cargarBannersJson() {
  try {
    const response = await fetch('/assets/banners.json'); // Asegúrate de que la ruta sea correcta
    if (!response.ok) throw new Error('No se pudo cargar banners.json');
    const data = await response.json();
    bannersJSON = data; // ✅ Asignación directa
    console.log("✅ bannersJSON cargado:", bannersJSON);
  } catch (err) {
    console.error('❌ Error cargando JSON:', err);
    mostrarToast("⚠️ Error al cargar banners.json", "danger");
  }
}
 
window.addEventListener("DOMContentLoaded", async () => {
  await cargarBannersJson();
  activarBotonLimpiar(); // y cualquier otra función de inicio
});

function sugerenciasBannerSimple(valor) {
  if (!Array.isArray(bannersJSON) || bannersJSON.length === 0) {
    return mostrarToast("⚠️ JSON aún no cargado", "warning");
  }
